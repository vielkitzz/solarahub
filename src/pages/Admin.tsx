    ...
    <div className="space-y-6 max-w-6xl mx-auto pb-10">
      ...
      <Tabs defaultValue="clubs" className="w-full">
        <TabsList className="bg-secondary/50 p-1 h-auto flex-wrap justify-start">
          ...
          <TabsTrigger value="transfer" className="gap-2">
            <ArrowRightLeft className="h-4 w-4" /> Mercado
          </TabsTrigger>
          <TabsTrigger value="empresas" className="gap-2">
            <TrendingUp className="h-4 w-4" /> Empresas
          </TabsTrigger>
            ...
            <div className="lg:col-span-1 space-y-4">
              <Card className="p-5 bg-gradient-card border-border/50 space-y-4">
                <h3 className="font-display font-bold flex items-center gap-2">
                  <Plus className="h-5 w-5 text-primary" /> Novo Clube
                </h3>
                <div className="space-y-3">
                  <div>
                    <Label>Nome do Clube *</Label>
                    <Input value={cName} onChange={(e) => setCName(e.target.value)} placeholder="Ex: Boca del Trueno" />
                  </div>
                  <div>
            ...
            <div className="lg:col-span-2 space-y-4">
              ...
              <div className="grid sm:grid-cols-2 gap-3">
                {filteredClubs.map((c) => (
                  <Card
                    key={c.id}
                    className="p-4 bg-gradient-card border-border/50 hover:border-primary/40 transition-all flex items-center gap-3 group"
                  >
                    <div className="h-12 w-12 flex items-center justify-center shrink-0 bg-secondary/30 rounded-lg">
                      {c.crest_url ? (
                        <img src={c.crest_url} alt={c.name} className="h-10 w-10 object-contain" />
                      ) : (
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-[10px] uppercase tracking-wider text-muted-foreground flex items-center gap-2">
                        <Users className="h-3 w-3" /> {playersByClub[c.id]?.length || 0} Elenco ·{" "}
                        {formatCurrency(c.budget)}
                      </div>
                    </div>
                    <div className="flex gap-1">
                      <Button size="icon" variant="ghost" onClick={() => setEditClub({ ...c })} className="h-8 w-8">
                ...
                {filteredClubs.length === 0 && (
                  <p className="col-span-full text-center py-10 text-muted-foreground">Nenhum clube encontrado.</p>
                )}
              </div>
            </div>
          </div>
              ...
              <div className="space-y-4 pt-4 border-t border-border/50">
                ...
                <div className="max-h-60 overflow-auto grid sm:grid-cols-2 gap-2">
                  {importPreview.map((p, i) => (
                    <div
                      key={i}
                      className="flex items-center gap-3 text-xs bg-secondary/40 rounded-lg p-2 border border-border/50"
                    >
            ...
            <div className="grid md:grid-cols-3 gap-4">
              ...
              <div className="md:col-span-3 space-y-2">
                <Label>
                  ...
                  <span className="text-muted-foreground font-normal">(Opcional: Ajusta caixas automaticamente)</span>
                </Label>
          ...
          <div className="grid md:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-[10px] text-muted-foreground mb-2 uppercase tracking-wider">
                    Atualmente: <span className="text-primary font-bold">{currentSeason}</span>
                  </p>
                  <div className="flex gap-2">
                    <Input
                      value={newSeasonName}
                    ...
                    />
                    <Button onClick={updateSeasonName} variant="outline">
                      Salvar
                    </Button>
                  </div>
                </div>
            ...
            <Card className="p-5 bg-gradient-card border-border/50 space-y-4">
              <h3 className="font-display font-bold flex items-center gap-2 text-destructive">
                <CalendarClock className="h-5 w-5" /> Virada de Temporada
              </h3>
          ...
          {seasonResult && (
            <Card className="p-5 bg-gradient-card border-border/50 space-y-4">
              <h4 className="font-bold flex items-center gap-2">
                <TrendingUp className="h-4 w-4 text-primary" /> Relatório da Virada ({seasonResult.length} Clubes)
              </h4>
              <div className="max-h-64 overflow-auto space-y-1">
                {seasonResult.map((r) => (
                  <div
                    key={r.club_id}
                    className="flex items-center justify-between text-xs bg-secondary/30 rounded px-3 py-2 border border-border/50"
                  >
        ...
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-destructive">
              <AlertTriangle className="h-5 w-5" /> Confirmar Virada de Temporada
            </DialogTitle>
          </DialogHeader>
          <p className="text-sm text-muted-foreground">
            Você está prestes a processar as finanças de todos os clubes da liga. Certifique-se de que todos os
            resultados e patrocínios foram devidamente lançados.
          </p>
          <DialogFooter>
      ...
      <Dialog open={!!editClub} onOpenChange={(o) => !o && setEditClub(null)}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Editar Clube: {editClub?.name}</DialogTitle>
          </DialogHeader>
          {editClub && (
            <div className="grid md:grid-cols-2 gap-4">
              <div className="space-y-2">
