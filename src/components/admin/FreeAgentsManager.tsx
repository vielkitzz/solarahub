...
export const FreeAgentsManager = () => {
  ...
  const load = async () => {
    const { data } = await supabase.from("free_agents").select("*").order("overall", { ascending: false });
    setRows((data as any) || []);
  };
  useEffect(() => {
    load();
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap gap-2 items-center justify-between">
        <h3 className="font-semibold">Passes Livres ({rows.length})</h3>
      ...
      <Dialog open={!!editing} onOpenChange={(o) => !o && setEditing(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{editing?.id ? "Editar" : "Novo"} passe livre</DialogTitle>
          </DialogHeader>
          {editing && (
