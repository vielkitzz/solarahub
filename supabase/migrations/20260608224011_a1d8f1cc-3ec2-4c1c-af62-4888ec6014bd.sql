...
-- Allow system-level operations (like season turnover loan returns) to bypass squad limits triggers
...
BEGIN
  -- Ignora em contexto de sistema (sem auth) OU quando explicitamente sinalizado
  ...
  SELECT COUNT(*) INTO total_elenco
  FROM public.players
  ...
  IF COALESCE(NEW.nationality, '') <> 'Solara' THEN
    SELECT COUNT(*) INTO total_estrangeiros
    FROM public.players
...
-- Patch process_season_turnover to set the bypass flag at the start
...
BEGIN
  ...
  -- Bypass limites de elenco durante a virada (devoluções de empréstimo, etc.)
